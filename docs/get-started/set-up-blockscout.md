...
Blockscout has its own [documentation](https://docs.blockscout.com/for-developers/manual-deployment), but it is not explained very well.
This guide will try to simplify the deployment process and provide you with the step-by-step instructions on how to run your own Blockscout instance and pull data from Polygon-Edge network.
