...
Used to specify the data directory used for storing Polygon SDK client data. Default: ``./test-chain`.
...
<h4><i>handler-config</i></h4>

<Tabs>
  <TabItem value="syntax" label="Syntax" default>

    server [--handler-config HANDLER_CONFIG]

  </TabItem>
  <TabItem value="example" label="Example">

    server --handler-config config.json

  </TabItem>
</Tabs>

Can be provided to configure the JSON-RPC and WS server handler.
The configurable field is `allowed_origins`.

Here is an example of a valid configuration file which allow all origins to perform a request :
