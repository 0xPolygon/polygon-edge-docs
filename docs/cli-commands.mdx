...
A more detailed explanation on the block gas target can be found in the [TxPool section](/docs/reference/modules/txpool#block-gas-target).
...
<h4>
  <i>account-pending-limit</i>
</h4>

<Tabs>
  <TabItem value="syntax" label="Syntax" default>

    server [--account-pending-limit ACCOUNT_PENDING_LIMIT]

  </TabItem>
  <TabItem value="example" label="Example">

    server --account-pending-limit 16
    
  </TabItem>
</Tabs>

Sets maximum number of transactions in pending queue per account. Default: `16`.
...
Sets the maximum amount of time a non-executable transaction can wait in the queue. Default: `3h`.
...
Instead of an IP address, DNS address of the bootnode can be provided.
...
| secrets generate         | Generates a secrets manager configuration file which can be parsed by the Polygon SDK    |
...
<Tabs>
  ...
  <TabItem value="example" label="Example">
    ...
    loadbot --account 0x1010101010101010101010101010101010101020
...
<Tabs>
  ...
  <TabItem value="example" label="Example">
    ...
    loadbot --url http://127.0.0.1:8545

  </TabItem>
</Tabs>

A JSON RPC endpoint used to send transactions. You can specify more to send transactions to many nodes during the
stress test.
